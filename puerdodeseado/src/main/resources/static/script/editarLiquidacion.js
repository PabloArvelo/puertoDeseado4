...
async function buscarLiquidacionesPorPeriodo(periodo) {
    try {
        const respuesta = await fetch(`http://localhost:8080/editarLiquidacion/liquidacionesPorPeriodo?periodo=${periodo}`)
        ...
        data.forEach(liquidacion => {
            ...
            imgVer.onclick = function () {             
                ...
                fetch(`http://localhost:8080/editarLiquidacion/guardarParametrosSesion`,{
                    method:"POST",
                    headers: {
